...
OhosWebPermissionDataBaseAdapterImpl& OhosWebPermissionDataBaseAdapterImpl::GetInstance(const std::string& cachePath)
{
    WVLOG_I("webdatabase get permission data base instance");
    static OhosWebPermissionDataBaseAdapterImpl instance(cachePath);
    return instance;
}
...
OhosWebPermissionDataBaseAdapterImpl::OhosWebPermissionDataBaseAdapterImpl(const std::string& cachePath)
{
    ...
    if (access(cachePath.c_str(), F_OK) != 0) {
        WVLOG_E("web permission fail to access cache web dir:%{public}s", cachePath.c_str());
        return;
    }
    ...
    WVLOG_I("web permission database databaseDir=%{public}s", cachePath.c_str());
