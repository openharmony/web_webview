    ...
    int32_t surfaceId, uint64_t& nodeId)
{
    ...
    if (reply.ReadBool(hasRSHandle) && hasRSHandle) {
        rsHandle = reply.ReadRemoteObject();
    } else {
        WVLOG_W("No RS handle returned.");
    }
    return { surface, rsHandle };
}
...
void* AafwkBrowserClientAdapterImpl::QueryRenderSurface(int32_t surface_id)
{
    ...
    if (GetInstance().browserHost_) {
        uint64_t node_id = 0;
        auto [surfaceObject, rsHandleObj] =
