...
void* SurfaceControlAdapterImpl::FromNativeWindow(void* parent, const char* name)
{
    auto* parentSurface = reinterpret_cast<OHNativeWindow*>(parent);
    return SurfaceControlUtils::FromNativeWindow(parentSurface, name);
}
...
void SurfaceControlAdapterImpl::SetWindowNodeIdAndRSHandle(void* nativeWindow, uint64_t nodeId, uintptr_t handle)
{
...
void* SurfaceTransactionAdapterImpl::Create()
{
    return SurfaceControlUtils::Transaction::Create();
}
...
void SurfaceTransactionAdapterImpl::SetFrame(void* transaction, void* surfaceControl,
    float x, float y, float w, float h)
{
    auto* txn = reinterpret_cast<OH_SurfaceTransaction*>(transaction);
    auto* surface = reinterpret_cast<OH_SurfaceControl*>(surfaceControl);
    SurfaceControlUtils::Transaction::SetFrame(txn, surface, x, y, w, h);
}
...
void SurfaceTransactionAdapterImpl::SetScale(
    void* transaction, void* surfaceControl, float scaleX, float scaleY, float scaleZ)
{
    auto* txn = reinterpret_cast<OH_SurfaceTransaction*>(transaction);
    auto* surface = reinterpret_cast<OH_SurfaceControl*>(surfaceControl);
    SurfaceControlUtils::Transaction::SetScale(txn, surface, scaleX, scaleY, scaleZ);
}
