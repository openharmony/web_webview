...
sptr<SurfaceControl> SurfaceControl::CreateFromWindow(NativeWindow* window, const char* name)
{
    ...
    std::string fetchConnectToRender = surface->GetUserData("delegate_connect_to_render");
    sptr<IRemoteObject> connectToRender = nullptr;
    if (!fetchConnectToRender.empty()) {
        uintptr_t handle = static_cast<uintptr_t>(std::stoull(fetchConnectToRender));
        if (handle != 0) {
    ...
    : surfaceNode_(std::move(surfaceNode)), parentNode_(std::move(parentNode))
{
    if (surfaceNode_) {
        ...
        WVLOG_E("surfaceNode is nullptr");
    }
}
...
{
    CHECK_NULL_POINTER(surfaceNode_);
    CHECK_NULL_POINTER(crop);
    RS_TRACE_NAME_FMT("SurfaceControl:%s, node:[name: %s, id: %" PRIu64 "], CropRect[x:%.2f, y:%.2f, w:%.2f, h:%.2f]",
        __func__, surfaceNode_->GetName().c_str(), surfaceNode_->GetId(), crop->x, crop->y, crop->w, crop->h);
    surfaceNode_->SetClipRRect({ crop->x, crop->y, crop->w, crop->h }, { 0, 0, 0, 0 });
}
...
void SurfaceControl::SetFrame(float x, float y, float w, float h)
{
    return;
}

void SurfaceControl::SetCrop(float x, float y, float w, float h)
{
    CHECK_NULL_POINTER(surfaceNode_);
    surfaceNode_->SetClipRRect({ x, y, w, h }, { 0, 0, 0, 0 });
}
...
void SurfaceControl::SetPosition(float x, float y)
{
    CHECK_NULL_POINTER(surfaceNode_);
    surfaceNode_->SetTranslate(x, y, 0);
}
...
void SurfaceControl::SetScale(float scaleX, float scaleY, float scaleZ)
{
    return;
}

void SurfaceControl::SetRotation(float degreeX, float degreeY, float degreeZ)
{
    CHECK_NULL_POINTER(surfaceNode_);
    surfaceNode_->SetRotation(degreeX, degreeY, degreeZ);
}
...
void SurfaceControl::SetSkew(float skewX, float skewY, float skewZ)
{
    CHECK_NULL_POINTER(surfaceNode_);
    surfaceNode_->SetSkew(skewX, skewY, skewZ);
}

void SurfaceControl::SetPerspective(float perspX, float perspY, float perspZ, float perspW)
{
    CHECK_NULL_POINTER(surfaceNode_);
    surfaceNode_->SetPersp(perspX, perspY, perspZ, perspW);
}
...
void SurfaceControl::SetFrameRateWithChangeStrategy(float frame_rate, int8_t compatibility, int32_t strategy)
{
    WVLOG_W("SurfaceControl::%{public}s is not implemented ", __func__);
}
