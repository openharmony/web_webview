    ...
    int32_t fd, size_t offset, size_t len, FileMapperType type)
{
    if (fd < 0 || len == 0 || type == FileMapperType::NORMAL_MEM) {
        WVLOG_D("Invalid param fileName: %{public}s", fileName.c_str());
        return false;
    }
    if (g_pageSize <= 0) {
        WVLOG_D("Wrong Pagesize: %{public}ld", g_pageSize);
        return false;
    }
    if (dataLen_ > 0) {
        WVLOG_D("data not empty fileName: %{public}s", fileName_.c_str());
        return false;
    }
    ...
    if (basePtr_ == MAP_FAILED) {
        WVLOG_D("mmap failed, errno[%{public}d]. fileName: %{public}s, "
...
{
    if (!fileReader) {
        WVLOG_D("file null fileName: %{public}s", fileName.c_str());
        return false;
    }
    if (!fileName_.empty()) {
        WVLOG_D("data not empty fileName: %{public}s", fileName_.c_str());
    ...
    if (!fileReader->ReadBuffer(dataPtr_.get(), offset, len)) {
        WVLOG_D("read failed, len[%{public}zu]. fileName: %{public}s, "
