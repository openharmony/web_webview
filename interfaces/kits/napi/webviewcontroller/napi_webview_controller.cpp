...
napi_value NapiWebviewController::Init(napi_env env, napi_value exports)
{
    ...
    napi_property_descriptor webEngineVersionProperties[] = {
        ...
        DECLARE_NAPI_STATIC_PROPERTY("M144", NapiParseUtils::ToInt32Value(env,
            static_cast<int32_t>(ArkWeb::ArkWebEngineVersion::M144))),
...
napi_value NapiWebviewController::CreateWebPrintDocumentAdapter(napi_env env, napi_callback_info info)
{
    ...
    (void)webviewController.release();
    return proxy;
}
    ...
    napi_env env, napi_callback_info info)
{
    ...
    WebviewController *webviewController = GetWebviewController(env, info);
    if (!webviewController) {
        WVLOG_E("EnableIntelligentTrackingPrevention failed for webviewController failed");
        return result;
    }
    ...
    napi_env env, napi_callback_info info)
{
    ...
    if (!webviewController) {
        WVLOG_E("IsIntelligentTrackingPreventionEnabled failed for webviewController failed");
        return result;
    }
...
void SetUrlTrustListWithWildcard(napi_env env, napi_value* argv,
    WebviewController* webviewController, std::string& urlTrustList)
{
...
napi_value NapiWebviewController::SetUrlTrustList(napi_env env, napi_callback_info info)
{
    ...
    if (urlTrustList.size() > MAX_URL_TRUST_LIST_STR_LEN) {
        WVLOG_E("url trust list len is too large.");
        BusinessError::ThrowErrorByErrcode(env, PARAM_CHECK_ERROR);
        return result;
    }
