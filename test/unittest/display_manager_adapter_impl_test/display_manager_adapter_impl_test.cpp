...
public:
    ...
    void OnFoldStatusChanged(FoldStatus status) override { lastStatus = status; }
    FoldStatus GetLastStatus() const { return lastStatus; }
private:
    FoldStatus lastStatus = FoldStatus::UNKNOWN;
};
...
/**
 * @tc.name: DisplayManagerAdapterImplTest_023.
 * @tc.desc: test FoldStatusListenerAdapterImpl OnFoldStatusChanged does not crash with nullptr listener.
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(DisplayManagerAdapterImplTest, DisplayManagerAdapterImplTest_023, TestSize.Level1)
{
    std::unique_ptr<FoldStatusListenerAdapterImpl> foldListenerAdapterImpl =
        std::make_unique<FoldStatusListenerAdapterImpl>(nullptr);
    ASSERT_NE(foldListenerAdapterImpl, nullptr);
    foldListenerAdapterImpl->OnFoldStatusChanged(DISPLAY_MANAGER_FOLD_DISPLAY_MODE_FULL);
    foldListenerAdapterImpl->OnFoldStatusChanged(DISPLAY_MANAGER_FOLD_DISPLAY_MODE_MAIN);
}

/**
 * @tc.name: DisplayManagerAdapterImplTest_024.
 * @tc.desc: test ConvertFoldStatus default branch.
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(DisplayManagerAdapterImplTest, DisplayManagerAdapterImplTest_024, TestSize.Level1)
{
    std::unique_ptr<FoldStatusListenerAdapterImpl> foldListenerAdapterImpl =
        std::make_unique<FoldStatusListenerAdapterImpl>(nullptr);
    EXPECT_EQ(foldListenerAdapterImpl->ConvertFoldStatus(static_cast<NativeDisplayManager_FoldDisplayMode>(100)),
                OHOS::NWeb::FoldStatus::UNKNOWN);
}

/**
 * @tc.name: DisplayManagerAdapterImplTest_025.
 * @tc.desc: test ConvertFoldStatus default branch in DisplayAdapterImpl.
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(DisplayManagerAdapterImplTest, DisplayManagerAdapterImplTest_025, TestSize.Level1)
{
    std::unique_ptr<DisplayAdapterImpl> displayAdapterImpl =
        std::make_unique<DisplayAdapterImpl>(nullptr);
    EXPECT_EQ(displayAdapterImpl->ConvertFoldStatus(static_cast<NativeDisplayManager_FoldDisplayMode>(100)),
                OHOS::NWeb::FoldStatus::UNKNOWN);
}

/**
 * @tc.name: DisplayManagerAdapterImplTest_026.
 * @tc.desc: test GetDpi when xDpi is valid and returns non-negative value.
 * @tc.type: FUNC
 * @tc.require:
 */
...
/**
 * @tc.name: DisplayManagerAdapterImplTest_027.
 * @tc.desc: test GetDefaultDisplay with VASSISTANT_SCREEN_NAME.
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(DisplayManagerAdapterImplTest, DisplayManagerAdapterImplTest_027, TestSize.Level1)
{
    std::shared_ptr<DisplayManagerAdapterImpl> displayManagerAdapterImpl =
        std::make_shared<DisplayManagerAdapterImpl>();
    ASSERT_NE(displayManagerAdapterImpl, nullptr);
    auto display = displayManagerAdapterImpl->GetDefaultDisplay();
    EXPECT_NE(display, nullptr);
}

/**
 * @tc.name: DisplayManagerAdapterImplTest_028.
 * @tc.desc: test DisplayAdapterImpl getters return default values when constructed with nullptr.
 * @tc.type: FUNC
 * @tc.require:
 */
 ...
 * @tc.desc: test GetDisplayOrientation with nullptr display.
 * @tc.type: FUNC
 * @tc.require:
 */
 ...
 * @tc.desc: test CheckOnlyRefreshRateDecreased returns false when cached refresh rate is higher.
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(DisplayManagerAdapterImplTest, DisplayManagerAdapterImplTest_030, TestSize.Level1)
{
    std::shared_ptr<DisplayListenerAdapterTest> listener =
        std::make_shared<DisplayListenerAdapterTest>();
    std::unique_ptr<DisplayListenerAdapterImpl> displayListenerAdapterImpl =
        std::make_unique<DisplayListenerAdapterImpl>(listener);
    auto displayPtr = DisplayManager::GetInstance().GetDefaultDisplay();
    ASSERT_NE(displayPtr, nullptr);
    ...
    auto nwebDisplayInfo = displayListenerAdapterImpl->ConvertDisplayInfo(*displayInfo);
    displayListenerAdapterImpl->cachedDisplayedInfo_ = nwebDisplayInfo;
    displayListenerAdapterImpl->cachedDisplayedInfo_.refreshRate_ = displayInfo->GetRefreshRate() + 10;
    auto displayId = DisplayManager::GetInstance().GetDefaultDisplayId();
    EXPECT_TRUE(displayListenerAdapterImpl->CheckOnlyRefreshRateDecreased(displayId));
    displayListenerAdapterImpl->OnChange(displayId);
}

/**
 * @tc.name: DisplayManagerAdapterImplTest_031.
 * @tc.desc: test FoldStatusListenerAdapterImpl with valid listener.
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(DisplayManagerAdapterImplTest, DisplayManagerAdapterImplTest_031, TestSize.Level1)
{
    std::shared_ptr<FoldStatusListenerAdapterTest> listener =
        std::make_shared<FoldStatusListenerAdapterTest>();
    std::unique_ptr<FoldStatusListenerAdapterImpl> foldListenerAdapterImpl =
        std::make_unique<FoldStatusListenerAdapterImpl>(listener);
    foldListenerAdapterImpl->OnFoldStatusChanged(DISPLAY_MANAGER_FOLD_DISPLAY_MODE_FULL);
    EXPECT_EQ(listener->GetLastStatus(), OHOS::NWeb::FoldStatus::FULL);
    foldListenerAdapterImpl->OnFoldStatusChanged(DISPLAY_MANAGER_FOLD_DISPLAY_MODE_MAIN);
    EXPECT_EQ(listener->GetLastStatus(), OHOS::NWeb::FoldStatus::MAIN);
    foldListenerAdapterImpl->OnFoldStatusChanged(DISPLAY_MANAGER_FOLD_DISPLAY_MODE_SUB);
    EXPECT_EQ(listener->GetLastStatus(), OHOS::NWeb::FoldStatus::SUB);
    foldListenerAdapterImpl->OnFoldStatusChanged(DISPLAY_MANAGER_FOLD_DISPLAY_MODE_COORDINATION);
    EXPECT_EQ(listener->GetLastStatus(), OHOS::NWeb::FoldStatus::COORDINATION);
    foldListenerAdapterImpl->OnFoldStatusChanged(
