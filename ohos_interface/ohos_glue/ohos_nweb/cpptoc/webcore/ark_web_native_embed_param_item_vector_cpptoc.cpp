...
{
    ArkWebNativeEmbedParamItemVector struct_value = { .size = class_value.size(),
    ...
    if (struct_value.size > 0) {
        ...
        for (auto it = class_value.begin(); it != class_value.end(); it++) {
            ...
            struct_value.value[count] = ArkWebNativeEmbedParamItemCppToC::Invert(ark_web_native_embed_param_item);
            count++;
        }
    }
