...
{
    ArkWebCookieManagerCookiesVector struct_value = { .size = class_value.size(), .ark_web_mem_free_func = ArkWebMemFree };
    if (struct_value.size > 0) {
        struct_value.value =
            (ark_web_cookie_manager_cookies_t**)ArkWebMemMalloc(sizeof(ark_web_cookie_manager_cookies_t*) * struct_value.size);
        ...
        for (auto it = class_value.begin(); it != class_value.end(); it++) {
            ArkWebRefPtr<ArkWebCookieManagerCookies> ark_web_cookie_manager_cookies = new ArkWebCookieManagerCookiesImpl(*it);
