...
void ArkWebEngineWrapper::LibraryLoaded(std::shared_ptr<OHOS::NWeb::NWebEngineInitArgs> init_args, bool lazy)
{
    if (CHECK_SHARED_PTR_IS_NULL(init_args)) {
        return;
    }
...
void ArkWebEngineWrapper::SetUserAgentClientHintsEnabled(bool enabled)
{
    ark_web_engine_->SetUserAgentClientHintsEnabled(enabled);
}
...
void ArkWebEngineWrapper::DumpArkWebJSHeap(int32_t fd, int32_t renderPid, bool needSnapshot, bool needGC, bool needRaw)
{
    ark_web_engine_->DumpArkWebJSHeap(fd, renderPid, needSnapshot, needGC, needRaw);
}
} // namespace OHOS::ArkWeb
